...
public class GameInfoDynamics {
 ...
 /** lastAction[i] last fold(0), call(1), or raise(2) made by the player in seat i, or -1 otherwise. */
  ...
  /** Two at the beginning of the stage, (before first bet or check) Zero at end of stage (after last call), one elsewhere */
	...
	public static final double smallBet = 2.0;

	public GameInfoDynamics() {
		double initialPot = 400d; // Christian K: doyles game has 400 bankroll
		bankroll = new double[2];
		bankroll[0] = initialPot;
		bankroll[1] = initialPot;
		bankrollAtStart = new double[2];
		bankrollAtStart[0] = initialPot;
		bankrollAtStart[1] = initialPot;
		hasActed = new boolean[2];
		doNewGame(0, 0);
	}

	public void doNewGame(long gameID, int button) {
		// JK: Fixed, as bankroll and bankrollAtStart were inverted
		bankroll[0] = bankrollAtStart[0];
		bankroll[1] = bankrollAtStart[1];
		this.gameID = gameID;
		this.button = button;
		gameOver = false;
		inPot = new double[2];
   ...
   */
	...
	/**
	 * Called in-between the action event and the state change Does not change
	 * the current seat
	 * 
	 * JK: For limit only, NL uses doPostBetOrRaise(double amount)
	 */
   ...
   * Does not change the current seat
   *
   * JK: For no-limit only
   */
   	public void doPostBetOrRaise(double amount) {
		lastAction[currentPlayerSeat] = Holdem.RAISE;
		roundBets++;
		addToPot(currentPlayerSeat, getAmountToCall(currentPlayerSeat) + amount);
		hasActed[currentPlayerSeat] = true;
		if (numToAct > 1) {
			numToAct--;
		}
